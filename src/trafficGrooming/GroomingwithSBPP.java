...
public class GroomingwithSBPP {
	...
	public void SBPPgrooming(Layer ipLayer, Layer optLayer) {
		...
		for (int i = 0; i < nodePairList.size(); i++) {
			...
			/**
			 * 在Ip层路由工作路径： 1：在IP层的每个链路，查找该链路的虚拟链路中的剩余容量最大的链路
			 * 2：如果该容量大于当前结点对的容量需求，则将对应虚拟链路的cost和length信息复制到链路中去 3：否则将该链路排除
			 * 4：在剩余的拓扑上路由该需求 5:如果路由成功，更新虚拟链路的容量信息，否则的话，看着办 6：否则的话，新建光路
			 */
			...
			if (newRoute0.getLinklist().size() != 0) {
				...
				/**
				 * 新建工作路径： 1：在光层上路由 2：分配频谱资源 3：新建光路
				 */
	...
	public void protectionGrooming(Node srcNode,Node DestNode,Layer ipLayer,Layer optLayer){
